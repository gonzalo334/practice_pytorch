...
class TestMaxOut:
    ...
    def test_forward(
        ...
        # Check dtype
        assert outputs.dtype == outputs_loop.dtype, (
            f"Incorrect outputs dtype, expected {outputs_loop.dtype} and got "
            f"{outputs.dtype}"
        )

        # Check values against the loop implementation
        max_abs_diff = torch.max(torch.abs(outputs - outputs_loop)).item()
        torch.testing.assert_close(
            outputs,
            outputs_loop,
            rtol=1e-10,
            atol=1e-10,
            msg=f"Forward output does not match reference implementation. "
